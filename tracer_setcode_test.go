...
func TestTracer_SetCodeAuthorization(t *testing.T) {
	...
	t.Run("multiple_authorizations_mixed_validity", func(t *testing.T) {
		...
		aliceAuth, err := SignSetCodeAuth(AliceKey, 1, CharlieAddr, 0)
		require.NoError(t, err)

		// Bob signs a valid authorization
		bobAuth, err := SignSetCodeAuth(BobKey, 1, CharlieAddr, 0)
		require.NoError(t, err)

		// Charlie signs an authorization (but we won't add nonce change - will be discarded)
		charlieAuth, err := SignSetCodeAuth(CharlieKey, 1, AliceAddr, 0)
		require.NoError(t, err)

		txEvent := new(TxEventBuilder).
			Defaults().
			Type(TxTypeSetCode).
			SetCodeAuthorizations([]SetCodeAuthorization{aliceAuth, bobAuth, charlieAuth}).
			Build()
	...
	t.Run("duplicate_nonce_change_only_one_used", func(t *testing.T) {
		...
		auth1, err := SignSetCodeAuth(AliceKey, 1, CharlieAddr, 0)
		require.NoError(t, err)

		// Create second authorization from same key (Alice) with same nonce
		auth2, err := SignSetCodeAuth(AliceKey, 1, BobAddr, 0)
		require.NoError(t, err)

		txEvent := new(TxEventBuilder).
			Defaults().
			Type(TxTypeSetCode).
			SetCodeAuthorizations([]SetCodeAuthorization{auth1, auth2}).
			Build()
