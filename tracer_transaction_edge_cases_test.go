...
func TestTracer_CompleteTransaction_BlobTransaction(t *testing.T) {
	t.Run("blob_transaction_with_blob_gas", func(t *testing.T) {
		...
		tester.Tracer.transaction.Type = pbeth.TransactionTrace_TRX_TYPE_BLOB
		...
		block := ParseFirehoseBlock(t, "shared tracer", tester.Tracer.config.OutputWriter.(*bytes.Buffer))
		trx := block.TransactionTraces[0]
		...
		// Verify BlobGasPrice - convert big.Int to protobuf format to compare
		expectedPrice := bigIntToProtobuf(blobGasPrice)
		assert.Equal(t, expectedPrice.Bytes, trx.Receipt.BlobGasPrice.Bytes,
