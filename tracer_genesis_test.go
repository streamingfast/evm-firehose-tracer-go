	...
	t.Run("single_account_with_balance", func(t *testing.T) {
		...
		NewTracerTester(t).
			...
			Validate(func(block *pbeth.Block) {
				...
				assert.Equal(t, AliceAddr[:], change.Address, "Address should be Alice")
	...
	t.Run("single_account_with_code", func(t *testing.T) {
		...
		NewTracerTester(t).
			...
			Validate(func(block *pbeth.Block) {
				...
				assert.Equal(t, BobAddr[:], change.Address, "Address should be Bob")
				assert.Equal(t, EmptyHash[:], change.OldHash, "Old hash should be empty")
	...
	t.Run("single_account_with_nonce", func(t *testing.T) {
		...
		NewTracerTester(t).
			...
			Validate(func(block *pbeth.Block) {
				...
				assert.Equal(t, CharlieAddr[:], change.Address, "Address should be Charlie")
	...
	t.Run("single_account_with_storage", func(t *testing.T) {
		...
		NewTracerTester(t).
			...
			Validate(func(block *pbeth.Block) {
				...
				assert.Equal(t, BobAddr[:], change0.Address, "Address should be Bob")
				assert.Equal(t, key1[:], change0.Key, "First key should be hash32(1)")
				assert.Equal(t, EmptyHash[:], change0.OldValue, "Old value should be empty")
