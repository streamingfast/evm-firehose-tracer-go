...
func TestTracer_OnOpcodeFault(t *testing.T) {
	t.Run("invalid_opcode_fault", func(t *testing.T) {
		...
		NewTracerTester(t).
			...
			StartRootCall(AliceAddr, BobAddr, bigInt(100), 21000, []byte{0xff, 0xfe}).
	...
	t.Run("stack_underflow_fault", func(t *testing.T) {
		...
		NewTracerTester(t).
			StartBlockTrx(TestLegacyTrx).
			StartRootCall(AliceAddr, BobAddr, bigInt(100), 21000, []byte{0x01}). // ADD without stack items
	...
	t.Run("stack_overflow_fault", func(t *testing.T) {
		...
		NewTracerTester(t).
			StartBlockTrx(TestLegacyTrx).
			StartRootCall(AliceAddr, BobAddr, bigInt(100), 100000, []byte{0x60, 0x00}). // PUSH1 0
	...
	t.Run("out_of_gas_fault", func(t *testing.T) {
		...
		NewTracerTester(t).
			StartBlockTrx(TestLegacyTrx).
			StartRootCall(AliceAddr, BobAddr, bigInt(100), 21000, []byte{0x5b}). // JUMPDEST
	...
	t.Run("multiple_opcode_faults_before_exit", func(t *testing.T) {
		...
		NewTracerTester(t).
			StartBlockTrx(TestLegacyTrx).
			StartRootCall(AliceAddr, BobAddr, bigInt(100), 21000, []byte{0x60, 0x01, 0xfe}).
